/****************************************************************************
 *
 * File:            toolboxsetopendestination.c
 *
...
int Usage()
{
    printf("Usage: toolboxsetopendestination <inputPath> <pageNumber> <outputPath>.\n");
    printf("       Example: in.pdf 2 out.pdf\n");
...
int _tmain(int argc, TCHAR* argv[])
{
    ...
    iPageCount = PtxPdf_PageList_GetCount(pInPageList);
    if (iDestPageNumber < 1 || iDestPageNumber > iPageCount)
    {
        _tprintf(_T("Given page number is invalid. Must be between 1 and %d.\n"), iPageCount);
        iReturnValue = 1;
        goto cleanup;
    }
    ...
    // Create LocationZoomDestination: page, left=0, top=pageHeight, zoom=NULL
    {
        double dLeft = 0.0;
