...
int Usage()
{
    printf("Usage: toolboxremoveglyphs <inputPath> <outputPath>.\n");
                                ...
                                TPtxPdf_Document* pOutDoc)
{
    ...
    while ((pInElement = PtxPdfContent_ContentExtractorIterator_GetValue(pIterator)) != NULL)
    {
        ...
        if (iType == ePtxPdfContent_ContentElementType_GroupElement)
        {
            ...
            // Recursively copy content and remove glyphs
            if (copyContentAndRemoveGlyphs(pInGroupContent, pOutGroupContent, pOutDoc) != 0)
                goto cleanup;
            ...
            if (iType == ePtxPdfContent_ContentElementType_TextElement)
            {
                ...
                if (pText != NULL)
                {
                    ...
                    for (int iFrag = 0; iFrag < nFragCount; iFrag++)
                    {
                        ...
                        if (pFragment != NULL)
                        {
                            int nGlyphCount = PtxPdfContent_TextFragment_GetCount(pFragment);
                            // Ensure that the fragment has more than two glyphs
                            if (nGlyphCount > 2)
                            {
                                // Call RemoveAt twice to remove first two glyphs
                                PtxPdfContent_TextFragment_Remove(pFragment, 0);
                                PtxPdfContent_TextFragment_Remove(pFragment, 0);
                            }
...
int _tmain(int argc, TCHAR* argv[])
{
    ...
    for (int iPage = 0; iPage < PtxPdf_PageList_GetCount(pInPageList); iPage++)
    {
        ...
        // Copy content and remove glyphs
        if (copyContentAndRemoveGlyphs(pInContent, pOutContent, pOutDoc) != 0)
            goto cleanup;
